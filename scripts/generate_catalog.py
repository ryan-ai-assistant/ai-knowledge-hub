...
for path in ROOT.rglob("*.md"):
    if any(p in path.parts for p in [".github", "catalog", "_inbox"]):
        continue
...
(CATALOG / "index.json").write_text(json.dumps(rows, indent=2), encoding="utf-8")
...
import re as _re
new_readme = _re.sub(f"{start}.*?{end}", f"{start}\n{table}\n{end}", readme, flags=_re.S)
